  ...
  const result = rows.map(row => {
    ...
    try {
      ...
      if (Array.isArray(parsedEntries) && parsedEntries.length > 0) {
        const entryWithImg = parsedEntries.find(e => e && (e.cdnImage || e.image));
        let rawImg = entryWithImg ? (entryWithImg.cdnImage || entryWithImg.image) : (parsedEntries[0].cdnImage || parsedEntries[0].image || null);
        if (entryWithImg && entryWithImg.cdnImage) {
          previewImage = entryWithImg.cdnImage;
        } else if (entryWithImg && entryWithImg.imageFilename) {
          previewImage = `https://cdn.jsdelivr.net/gh/AllensCreations/ElderSalviejo@main/vault/gallery/photos/${entryWithImg.imageFilename}`;
        } else if (rawImg && rawImg.startsWith('data:image/') && rawImg.length > 50000) {
          // Do not send bulky base64 images in archive summary list
          previewImage = null;
        } else {
          previewImage = rawImg;
        }
