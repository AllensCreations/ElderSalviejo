...
// High-performance in-memory cache for Turso API responses (60s TTL)
let _weeksCache = { data: null, expiresAt: 0 };
let _galleryCache = { data: null, expiresAt: 0 };
let _bookCache = { data: null, expiresAt: 0 };

function invalidateApiCaches() {
  _weeksCache = { data: null, expiresAt: 0 };
  _galleryCache = { data: null, expiresAt: 0 };
  _bookCache = { data: null, expiresAt: 0 };
}
...
async function getAllWeeks() {
  const now = Date.now();
  if (_weeksCache.data && now < _weeksCache.expiresAt) {
    return _weeksCache.data;
  }
  ...
  // Never leave vault empty if sample-payload exists
  if (rows.length === 0) {
    try {
      const samplePath = path.join(__dirname, '..', 'public', 'sample-data', 'sample-payload.json');
      if (fs.existsSync(samplePath)) {
        const sampleRaw = fs.readFileSync(samplePath, 'utf8');
        const sampleObj = JSON.parse(sampleRaw);
        rows = [{
          id: 1,
          slug: sampleObj.slug || 'week-1',
          title: sampleObj.title || 'Week 1 in Dumaguete',
          published_at: sampleObj.publishedAt || new Date().toISOString(),
          raw_subject: sampleObj.rawSubject || '',
          sender: sampleObj.sender || 'Elder Salviejo',
          total_entries: sampleObj.totalEntries || 7,
          image_count: sampleObj.imageCount || 7,
          verse: sampleObj.verse || null,
          entries: sampleObj.entries || [],
          created_at: new Date().toISOString()
        }];
      }
    } catch (_) {}
  ...
  const result = rows.map(row => {
    ...
    try {
      ...
      if (Array.isArray(parsedEntries) && parsedEntries.length > 0) {
        const entryWithImg = parsedEntries.find(e => e && (e.cdnImage || e.image || e.imageFilename));
        if (entryWithImg && entryWithImg.imageFilename) {
          previewImage = `/vault/gallery/photos/${entryWithImg.imageFilename}`;
        } else if (entryWithImg && entryWithImg.cdnImage) {
          previewImage = entryWithImg.cdnImage;
        } else if (entryWithImg && entryWithImg.image && !entryWithImg.image.startsWith('data:image/')) {
          previewImage = entryWithImg.image;
        }
        snippet = extractMissionaryHighlightSnippet(parsedEntries);

        entryThumbnails = parsedEntries.map((e, idx) => {
          let thumb = '';
          if (e.imageFilename) {
            thumb = `/vault/gallery/photos/${e.imageFilename}`;
          } else if (e.cdnImage) {
            thumb = e.cdnImage;
          } else if (e.image && !e.image.startsWith('data:image/')) {
            thumb = e.image;
          }
          return {
            day: e.day || `Day ${idx + 1}`,
            time: e.time || '12:00 PHT',
            text: e.text || '',
            image: thumb,
    ...
    return {
      ...
      previewImage,
      snippet,
      entries: entryThumbnails
    };
  });

  _weeksCache = {
    data: result,
    expiresAt: now + 60000 // 60s cache
  };
  return result;
}

/**
 * Fetches all weekly journals with full parsed entries for the compiled mission record book / PDF maker.
 * Returns sorted chronologically (ascending: earliest week first for sequential reading).
 */
async function getFullBookWeeks() {
  const now = Date.now();
  if (_bookCache.data && now < _bookCache.expiresAt) {
  ...
  if (isTursoConfigured()) {
    try {
      ...
      console.warn('Turso query for getFullBookWeeks failed, using local vault:', dbErr.message);
      rows = readLocalDb();
      rows.sort((a, b) => new Date(a.published_at) - new Date(b.published_at));
    }
  } else {
  ...
  // Filter out temporary test/diagnostic entries so the keepsake book only compiles legitimate field letters
  const validRows = rows.filter(r => r && (!r.slug || !r.slug.startsWith('test-')));
  ...
  const result = rows.map(row => {
    ...
    return {
      ...
      rawSubject: row.raw_subject,
      sender: row.sender,
      totalEntries: row.total_entries || parsedEntries.length,
...
async function getAllGalleryPhotos() {
  const now = Date.now();
  if (_galleryCache.data && now < _galleryCache.expiresAt) {
  ...
  // Fallback to vault/gallery/index.json if database yields no photos

  if (photos.length === 0) {
    const vaultIndexPath = path.join(__dirname, '..', 'vault', 'gallery', 'index.json');
    if (fs.existsSync(vaultIndexPath)) {
      try {
        const vaultPhotos = JSON.parse(fs.readFileSync(vaultIndexPath, 'utf8'));
        if (Array.isArray(vaultPhotos)) {
          for (const item of vaultPhotos) {
            photos.push({
              id: item.id || `vault-${item.filename}`,
              src: item.src || `/vault/gallery/photos/${item.filename}`,
              localSrc: item.localSrc || `/vault/gallery/photos/${item.filename}`,
              cdnSrc: item.cdnSrc,
              legacyCdnSrc: item.legacyCdnSrc,
              filename: item.filename,
              date: item.dateTime || item.date,
              dateTime: item.dateTime,
              capturedDate: item.capturedDate,
              capturedTime: item.capturedTime,
              capturedDateTime: item.capturedDateTime,
              archivalStamp: item.archivalStamp,
              isExif: Boolean(item.isExif),
              category: item.category || 'Mission',
              caption: item.caption || '',
              album: item.album || 'Elder Salviejo Gallery',
              slug: item.slug || '',
              isGalleryUpload: true,
              source: 'gallery'
            });
  ...
  // Enrich photos with dimensions & orientation from vault index
  try {
    const vaultIndexPath = path.join(__dirname, '..', 'vault', 'gallery', 'index.json');
    if (fs.existsSync(vaultIndexPath)) {
      const vaultMeta = JSON.parse(fs.readFileSync(vaultIndexPath, 'utf8'));
      const metaMap = new Map();
      if (Array.isArray(vaultMeta)) {
        for (const m of vaultMeta) {
          if (m.filename) metaMap.set(m.filename, m);
        }
      }
      for (const p of photos) {
        const fname = p.filename || (p.src ? path.basename(p.src) : null);
        if (fname && metaMap.has(fname)) {
          const m = metaMap.get(fname);
          p.width = p.width || m.width || 600;
          p.height = p.height || m.height || 800;
          p.aspectRatio = p.aspectRatio || m.aspectRatio || (p.width / p.height);
          p.orientation = p.orientation || m.orientation || (p.width > p.height ? 'landscape' : 'portrait');
        } else if (p.width && p.height) {
          p.aspectRatio = p.aspectRatio || Number((p.width / p.height).toFixed(4));
          p.orientation = p.orientation || (p.width > p.height ? 'landscape' : 'portrait');
        }
      }
    }
  } catch (_) {}
  ...
  photos.sort((a, b) => new Date(b.date || b.dateTime || 0) - new Date(a.date || a.dateTime || 0));
...
async function exportCompleteDatabase() {
  ...
  if (isTursoConfigured()) {
    ...
    const subsPath = path.join(LOCAL_DATA_DIR, 'subscribers.json');
    if (fs.existsSync(subsPath)) {
      try { backup.tables.subscribers = JSON.parse(fs.readFileSync(subsPath, 'utf8')); } catch (_) { backup.tables.subscribers = []; }
    } else {
      backup.tables.subscribers = [];
    }
    const encPath = path.join(LOCAL_DATA_DIR, 'encouragements.json');
    if (fs.existsSync(encPath)) {
      try { backup.tables.family_encouragements = JSON.parse(fs.readFileSync(encPath, 'utf8')); } catch (_) { backup.tables.family_encouragements = []; }
    } else {
      backup.tables.family_encouragements = [];
    }
  }
