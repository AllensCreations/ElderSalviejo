  ...
  const result = rows.map(row => {
    ...
    try {
      ...
      if (Array.isArray(parsedEntries) && parsedEntries.length > 0) {
        ...
        // Protect payload: if image is raw base64 > 35KB, avoid sending massive blob in summary list
        if (rawImg && rawImg.startsWith('data:image/') && !rawImg.startsWith('data:image/svg') && rawImg.length > 35000) {
          previewImage = null; // Lightweight: openDocket lazy-fetches full entry from /api/weeks/[id]
        } else {
          previewImage = rawImg;
        }
