...
/**
 * Curated starter mission photos of Elder Salviejo
 */
const DEFAULT_MISSION_PHOTOS = [
  {
    id: 'starter-1',
    src: '/assets/images/elder-salviejo.jpg',
    date: '2026-03-01'
  },
  {
    id: 'starter-2',
...
async function getAllGalleryPhotos() {
  ...
  // 1. Collect photos from database entries (both 073000 gallery uploads and 159266 diary entries)
  for (const row of rows) {
    ...
    if (Array.isArray(entries)) {
      entries.forEach((entry, idx) => {
        ...
        if (imgSrc && !seenSrcs.has(imgSrc)) {
          seenSrcs.add(imgSrc);
          photos.push({
            id: `db-${row.slug || row.id}-${idx}`,
            src: imgSrc,
  ...
  // 2. Append starter mission photos if not already present
  for (const starter of DEFAULT_MISSION_PHOTOS) {
    if (!seenSrcs.has(starter.src)) {
      seenSrcs.add(starter.src);
      photos.push(starter);
    }
  }
