...
async function initDatabase() {
  if (isTursoConfigured()) {
    ...
    try {
      ...
      // 3. Family & Friends Encouragement Notes table
      await db.execute(`
...
async function getMissionStats() {
  ...
  if (isTursoConfigured()) {
    try {
      const db = getDbClient();
      const weeksRes = await db.execute(`SELECT COUNT(*) as count FROM journal_weeks WHERE is_gallery = 0;`);
      weeksCount = weeksRes.rows[0] ? Number(weeksRes.rows[0].count) : 0;

      const dateRes = await db.execute(`SELECT MIN(published_at) as earliest FROM journal_weeks WHERE is_gallery = 0;`);
      firstPublishedAt = dateRes.rows[0] ? dateRes.rows[0].earliest : null;
    } catch (_) {}
  } else {
    const weeks = readLocalDb();
    const diaryWeeks = weeks.filter(w => !w.is_gallery);
    weeksCount = diaryWeeks.length;
    if (diaryWeeks.length > 0) {
      firstPublishedAt = diaryWeeks[diaryWeeks.length - 1].published_at;
    }
  }
