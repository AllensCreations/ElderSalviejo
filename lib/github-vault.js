...
async function autoSaveToGitHub(payload) {
  ...
  try {
        ...
        try {
          ...
          if (match) {
            ...
            if (blobRes.ok) {
              ...
              // Every photo from 073000 AND 159266 automatically gets placed here!
              ...
              newGalleryPhotos.push({
                ...
                source: payload.isGallery ? '073000' : '159266',
                slug: payload.slug
              });

              // B. For weekly diaries (159266), also maintain vault/photos/
    ...
    // 5. If weekly diary (159266), generate Markdown, structured JSON, and update vault/index.json
    if (!payload.isGallery) {
